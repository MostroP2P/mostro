...
impl CashuClient {
    /// Connects to a mint URL and verifies it is reachable and that it
    /// supports the NUT-11 P2PK spending conditions the 2-of-3 escrow needs.
    /// A connection failure is surfaced to the caller via `Err` rather than a
    /// process-global flag, so the daemon can decide how to react at startup.
    pub async fn connect(mint_url: &str) -> Result<Self, Error> {
        ...
        let client = cdk::HttpClient::new(url, None);
        let cashu_client = Self { client };
