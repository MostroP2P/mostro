...
- Dev fee payment lifecycle in `src/app/dev_fee.rs`: `run_dev_fee_cycle()` (entry point), `send_dev_fee_payment()`, `resolve_dev_fee_invoice()`, idempotency checks, timeout handling, and crash recovery. The scheduler (`src/scheduler.rs`) calls `run_dev_fee_cycle()` once per tick; all state-machine logic lives in the dev_fee module.
...
- **Database field updates:**
  ...
  - `dev_fee_paid`: Changed from 0 to 1 after successful payment in `run_dev_fee_cycle`
...
- Event publishing from `src/app/dev_fee.rs` after successful payment (within `run_dev_fee_cycle`)
...
3. **Payment Execution** (`src/app/dev_fee.rs`)
   - `run_dev_fee_cycle()`: Entry point invoked by scheduler each tick; orchestrates find-unpaid, resolve, send, DB updates, audit events
   - `send_dev_fee_payment()`, `resolve_dev_fee_invoice()`: LNURL resolution and payment
   - Integration: `release.rs` enqueues orders for dev fee processing; scheduler calls `run_dev_fee_cycle()`
...
2. **Scheduler Processing** (`src/scheduler.rs::job_process_dev_fee_payment()` calls `src/app/dev_fee.rs::run_dev_fee_cycle()`):
   - Runs every 60 seconds
   - `run_dev_fee_cycle()` uses `find_unpaid_dev_fees()` to query database for orders where: `(status = 'settled-hold-invoice' OR status = 'success') AND dev_fee > 0 AND dev_fee_paid = 0`
...
Implementation in `src/app/dev_fee.rs::send_dev_fee_payment()`:
...
**When Changed to `1` (true):** After successful dev fee payment in `run_dev_fee_cycle()` (`src/app/dev_fee.rs`)
...
5. On payment success, `run_dev_fee_cycle()` updates: `order.dev_fee_paid = true` (stored as `1` in database)
...
The scheduler calls `run_dev_fee_cycle()` from `src/app/dev_fee.rs`; the implementation there includes enhanced logging. The scheduler wrapper remains thin:

```rust
/// Process unpaid development fees for successful orders
/// Called every 60 seconds by scheduler; delegates to dev_fee::run_dev_fee_cycle()
...
- [x] Scheduler job `job_process_dev_fee_payment()` in `src/scheduler.rs` calls `run_dev_fee_cycle()` in `src/app/dev_fee.rs`
  - Uses `find_unpaid_dev_fees()` database function in `src/db.rs`
  - Query: `SELECT * FROM orders WHERE (status = 'settled-hold-invoice' OR status = 'success') AND dev_fee > 0 AND dev_fee_paid = 0`
  - **Key improvement:** Query checks BOTH statuses to handle race conditions (commit 102cfed)
  - For each unpaid order (inside `run_dev_fee_cycle()`):
...
**Location:** `src/app/dev_fee.rs` (within `run_dev_fee_cycle()`, after payment success)
...
3. ✅ Integrated event publishing in `src/app/dev_fee.rs` after successful payment
