...
pub async fn start_scheduler(rate_list: Arc<Mutex<Vec<Event>>>, stats: Arc<Mutex<MostroMessageStats>>) {
...
async fn job_print_stats(stats: Arc<Mutex<MostroMessageStats>>) {
    ...
    tokio::spawn(async move {
        loop {
            info!("Stats on Mostro messages\r\n{:?}", *inner_stats.lock().await);
            let s = serde_json::to_string(&*inner_stats.lock().await);
            if let Ok(mut f) = File::open("mostro_stats.json") {
