...
async fn main() -> Result<()> {
    ...
    if lndk_client.is_some() {
        ...
        // bits LNDK needs. The check is informative only — bits 38/39 confirm
        // LND was built with onion-message support, but they don't guarantee
        // the operator actually started LND with the required protocol
        // flags. See docs/LNDK_SETUP.md for the full set.
    ...
    if RpcServer::is_enabled() {
        ...
        let rpc_lndk = lndk_client.clone();

        tokio::spawn(async move {
            match rpc_server
                .start(rpc_keys, rpc_pool, rpc_ln_client, rpc_lndk)
                .await
            {
