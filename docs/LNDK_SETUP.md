...
> **LNDK version pin.** Mostro vendors `proto/lndkrpc.proto` from
> [LNDK v0.3.0](https://github.com/lndk-org/lndk/releases/tag/v0.3.0).
> Run a matching LNDK release; newer versions may rename or remove fields
> and would silently break BOLT12 payouts at runtime.
...
**Strongly recommended.** A compromised LNDK with `admin.macaroon` would
mean a compromised LND. Bake a minimally scoped macaroon and point
`lndk_macaroon_file` at that file instead:
...
# Override only when LNDK runs on a remote host (must match the cert CN/SAN).
lndk_tls_domain = "localhost"
...
lndk_min_invoice_expiry = 60
# lndk_fee_limit_percent = 0.002  # fraction. Defaults to mostro.max_routing_fee.
```
