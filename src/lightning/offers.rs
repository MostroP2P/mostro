...
/// format. `None` for variants that validation rejects (`Unknown`, and
/// pre-fetched `Bolt12Invoice` since buyers must send an offer instead).
pub fn kind_tag(fmt: InvoiceFormat) -> Option<&'static str> {
    match fmt {
        ...
        InvoiceFormat::Bolt12Invoice | InvoiceFormat::Unknown => None,
    }
}
...
/// - Cannot be satisfied with `quantity = 1`
///
...
pub fn validate_offer(
    ...
    match offer.amount() {
        ...
        Some(Amount::Bitcoin { amount_msats }) => {
            // For zero-amount orders the buyer side is not paid out (no
            // amount to send), so an offer with a fixed amount has nothing
            // to validate against and reaching this branch would mean the
            // order is already malformed. Reject defensively.
            let expected =
                expected_amount_sats.ok_or(MostroInternalErr(ServiceError::InvoiceInvalidError))?;
    ...
    // Mostro pays a single unit per offer. Offers that require a minimum
    // quantity > 1 cannot be satisfied; LNDK's GetInvoice would later fail
    // with a confusing error, so reject up front.
...
mod tests {
    ...
    fn kind_tags() {
        ...
        assert_eq!(kind_tag(InvoiceFormat::Bolt12Invoice), None);
