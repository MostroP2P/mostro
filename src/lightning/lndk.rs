...
pub struct LndkConnector {
    ...
    /// Fee tolerance as a fraction of the payment amount (e.g. 0.002 = 0.2%).
    /// Converted to absolute msats per-payment so precision is preserved
...
impl LndkConnector {
    ...
    pub async fn new_from_settings() -> Result<Option<Self>, MostroError> {
        ...
        // LNDK's self-signed cert is issued for `localhost` by default; the
        // configured grpc host may be an IP (127.0.0.1) so we pin the SNI
        // / verification domain explicitly. `lndk_tls_domain` lets remote
        // deployments override the default.
        let tls = ClientTlsConfig::new()
            .ca_certificate(Certificate::from_pem(&cert))
            .domain_name(ln.lndk_tls_domain.clone());
        ...
        // Fee tolerance is stored as a fraction (0.002 = 0.2%) and converted
        // to absolute msats per-payment. `lndk_fee_limit_percent` is named
        // for parallelism with `max_routing_fee` (also a fraction); LNDK's
        // wire-level `fee_limit_percent` field is intentionally not used
        // because it's a whole-number percent and would round small fees up
        // by ~10x for typical `max_routing_fee` values.
        let fee_fraction = ln
    ...
    pub async fn pay_offer_validated(
        ...
        // LNDK's `fee_limit` field is in msats (uint32). Clamp at u32::MAX so
        // a pathological large amount can't overflow the wire encoding; in
        // practice payouts are well under 2^32 msats (~4.3 BTC).
        let fee_limit_msats = ((amount_msats as f64) * self.fee_fraction)
            .ceil()
        ...
        let mut pay_req = Request::new(PayInvoiceRequest {
            ...
            amount: Some(amount_msats),
            fee_limit: Some(fee_limit_msats),
